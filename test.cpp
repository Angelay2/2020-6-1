...
vector接口:
	...
	迭代器失效: 迭代器指向的位置已经释放或者越界  
		只要空间发生了变化,迭代器一定会失效(insert ,push_back,reserve,resize(修改size大小),assign,emplace_back)
		erase可能会导致迭代器越界访问, 故此接口也会导致迭代器失效 

如何避免迭代器失效?????????????????
	如果发生了空间变化, 重新获取迭代器(调一下begin或end), 
	如果是删除操作, 获取删除操作的返回值

	find: 不是vector的成员函数, 属于alogrithm库的函数模板里的全局函数
